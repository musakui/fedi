...
export const verifyRequest = async (req) => {
  ...
  if (req.headers.date) {
    const evtDate = new Date(req.headers.date)
    const dt = Math.abs(new Date() - evtDate)
    if (!(dt < 1e5)) throw new Error(`invalid date: ${req.headers.date} (${dt})`)
  }
  ...
  if (!sig.keyId || !sig.algorithm || !sig.headers || !sig.signature) {
    throw new Error(`invalid signature: ${req.headers.signature}`)
  }

  if (req.body && req.headers.digest) {
    const [ha, ...hd] = req.headers.digest.split('=')
    if (hd.join('=') !== hashDigest(req.body, ha)) {
      throw new Error(`invalid digest: ${req.headers.digest}`)
    }
  }
  ...
  const target = `${req.method} ${req.path}`.toLowerCase()
  const veri = createVerify(sig.algorithm)
  veri.update(sig.headers.split(' ').map((h) => `${h}: ${
    h === requestTarget ? target : req.headers[h]
  }`).join('\n'))
